...
def Type(object_str):
    ...
    screen = pygame.display.set_mode((600, 400))  # 打字框
    pygame.display.set_caption('type the following words')  # 命名
    pygame.display.update()


    clock= pygame.time.Clock()
    font = pygame.font.Font('NotoSansMonoCJKtc-Bold.otf', 60)  # 字體設置
    background = pygame.image.load('background3.png').convert_alpha()  # 背景
...
class Handcuff():
    def Catch(self):
        ...
        for k in killer_hit_list:
            Type('killer')
            killer_list.remove(k)
            current_goal += 500
        bad_hit_list = pygame.sprite.spritecollide(self, bad_list, False)
        for b in bad_hit_list:
            Type('bad')
            bad_list.remove(b)
            current_goal += 250
        scammer_hit_list = pygame.sprite.spritecollide(self, scammer_list, False)
        for s in scammer_hit_list:
            Type('scammer')
            scammer_list.remove(s)
            current_goal += 100
        stealer_hit_list = pygame.sprite.spritecollide(self, stealer_list, False)
        for s in stealer_hit_list:
            Type('stealer')
            bad_list.remove(s)
            current_goal += 50
        drink_hit_list = pygame.sprite.spritecollide(self, drink_list, False)
        for d in drink_hit_list:
            Type('drink')
            bad_list.remove(d)
            current_goal += 600
        triangle_hit_list = pygame.sprite.spritecollide(self, triangle_list, False)
        for t in triangle_hit_list:
            Type('triangle')
            bad_list.remove(t)
            current_goal += 30
        tnt_hit_list = pygame.sprite.spritecollide(self, tnt_list, False)
        for t in tnt_hit_list:
            Type('tnt')
            bad_list.remove(t)
            current_goal += 1
        reporter_hit_list = pygame.sprite.spritecollide(self, reporter_list, False)
        for r in reporter_hit_list:
            Type('reporter')
            bad_list.remove(r)
            current_goal += 2
