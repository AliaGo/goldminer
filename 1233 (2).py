...
def boom():
    boom_image = pygame.image.load('—Pngtree—explosion_571947.png').convert_alpha()
    boom_image = pygame.transform.scale(boom_image, (120, 120))
    ...
    for i in range(len(listxy) - 1):
        if listxy[i][0] not in now_x:
            if listxy[i][1] not in now_y:
                a = listxy[i][0]
                b = listxy[i][1]
                listxy.pop(i)
...
dt = 0
now_level = 0
criminal_list, tnt_list = Level.Criminal(now_level + 1)
